...
public class viewFormDetailServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try  {
            ...
            AppointmentDDD appointment_detail_NOTCHOOSE = null;
//            AppointmentDDD appointment_detail_CHOOSE = null;
            
            if (!consultation_id.isEmpty() && appointmentDAO.getRequestTrainerID(consultation_id) == null) {
                appointment_detail_NOTCHOOSE = appointmentDAO.getAppointDetailCUSnotchooseTRAINER(consultation_id);
            }else if(!consultation_id.isEmpty() && !appointmentDAO.getRequestTrainerID(consultation_id).isEmpty()){
                appointment_detail_NOTCHOOSE = appointmentDAO.getAppointDetailCUSchooseTRAINER(consultation_id);
            }
            
            HttpSession session =  request.getSession(true);
            session.setAttribute("appointment_detail_NOTCHOOSE", appointment_detail_NOTCHOOSE);
//            session.setAttribute("appointment_detail_CHOOSE", appointment_detail_CHOOSE);

...
//                + "						where T.status = 'available'";
     ...
     public void addNewTrainer(int trainer_id, int user_id, String fullname, String img, String contact)
            throws ClassNotFoundException, SQLException, IOException {
        String sql = "insert into tbl_trainer(trainer_id, user_id, fullname, achievement, img, status, contact)\n"
        ...
        try {
            ...
            ps.setBytes(4, Base64.getDecoder().decode(img));
    ...
    public int generateTrainerID() throws ClassNotFoundException, SQLException, IOException {
        String sql = "SELECT * FROM tbl_trainer WHERE trainer_id = (SELECT MAX(TRY_CAST(bird_id AS INT))FROM tbl_trainer)";
        int newID = 0;
        try {
            con = db.getConnection();
            ps = con.prepareStatement(sql);
            rs = ps.executeQuery();
            if (rs.next()) {
                int MAXconsultationID = rs.getInt("trainer_id");
...
public void setWorkshopComplete(String trainer_id, Date complete_date) throws ClassNotFoundException, SQLException {
        Connection con = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
...
public void setDataOnTableWorkshop(String trainer_id, String course_id, Date complete_date) throws ClassNotFoundException, SQLException {
        ...
        String sql = "INSERT INTO tbl_history_workshop_training\n"
                + "	(course_id, trainer_id, status, complete_date)\n"
                + "	VALUES\n"
                + "	(?, ?, 'Complete', ?)";
        try {
            con = db.getConnection();

...
public class BookingDAO implements Serializable {
    ...
    public void addNewBooking(int customer_id, int bird_id, int course_id, int amount) throws SQLException {
        LocalDate startDate = LocalDate.now();
        LocalDate endDate = startDate.plusDays(10);
        String sql = "Insert into tbl_Booking(trainer_id, course_id, customer_id, bird_id, start_date, end_date, status, amount, payment_id)\n"
    ...
    public List getPendingOrder() throws SQLException {
        ...
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
                BookingDTO booking = new BookingDTO(rs.getString(1), rs.getString(4), rs.getString(3), rs.getString(2));
                list.add(booking);
            }
        } catch (SQLException ex) {
            ex.printStackTrace();
    ...
    public void setTrainerToBooking(int trainer_id, int bird_id) throws SQLException {
        ...
        String sql = "Update tbl_Booking Set trainer_id = ?,  start_date = ?, end_date = ?, status = 'approved'\n"
                + "Where bird_id = ? ";
    ...
    public BookingDTO getCourseByBirdID(int bird_id) throws SQLException, ClassNotFoundException {
        ...
        try {
            ...
            rs = ps.executeQuery();
            if (rs.next()) {
                booking = new BookingDTO(rs.getString(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getInt(5));
            }
        } finally {
            if (rs != null) {
                rs.close();
